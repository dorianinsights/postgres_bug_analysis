# pyright: basic
"""Disclosed AI involvement per committed fix (fix_key), from a local LLM
(Ollama) reading int_commit_ai_texts -- classify-once, cached by the text's
CONTENT HASH in the committed data/raw/commit_ai_involvement.csv (the
...
def model(dbt: Any, session: Any) -> pa.Table:
    ...
    texts = dbt.ref("int_commit_ai_texts").project("fix_key, ai_text").fetchall()
    cache: dict[str, tuple[Any, ...]] = {}
    for r in dbt.source("inferred", "commit_ai_involvement").project(", ".join(LABEL_COLS)).fetchall():
        cache[str(r[0])] = tuple(r)

    rows = label_rows((((str(r[0]),), str(r[1])) for r in texts), cache, MODEL_TAG, classify, max_inline=max_inline)
