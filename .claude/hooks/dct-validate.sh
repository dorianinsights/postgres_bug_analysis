...
# from the cwd, so this runs from the repo root (where dbt_charts.yml sits beside
# dbt_project.yml); the venv is one level up at ../venv.

set -u

# cd to the repo root (the dct + dbt project root), resolved from this script's own
